...
use HBP_Disabler_Vendor\Hybrid\Contracts\MessageBag as MessageBagContract;
use HBP_Disabler_Vendor\Hybrid\Contracts\MessageProvider;
...
class MessageBag implements Jsonable, JsonSerializable, MessageBagContract, MessageProvider
{
    ...
    {
        return collect($this->messages)->filter(static fn($messages, $messageKey) => Str::is($key, $messageKey))->map(fn($messages, $messageKey) => $this->transform($messages, $this->checkFormat($format), $messageKey))->all();
    }
    /**
    ...
    protected function transform($messages, $format, $messageKey)
    {
        ...
        return collect((array) $messages)->map(static fn($message) => str_replace([':message', ':key'], [$message, $messageKey], $format))->all();
    }
    /**
