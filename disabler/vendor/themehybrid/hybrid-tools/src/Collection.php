...
class Collection implements ArrayAccess, CanBeEscapedWhenCastToString, Enumerable
{
     ...
     *
     * @param  \Hybrid\Contracts\Arrayable<TKey, TValue>|iterable<TKey, TValue>|null $items
     * @return void
     */
     ...
     *
     * @return \Hybrid\Tools\LazyCollection<TKey, TValue>
     */
    ...
    public function avg($callback = null)
    {
        ...
        $items = $this->map(static fn($value) => $callback($value))->filter(static fn($value) => !is_null($value));
    ...
    {
        $values = (isset($key) ? $this->pluck($key) : $this)->filter(static fn($item) => !is_null($item))->sort()->values();
        $count = $values->count();
        if ($count === 0) {
            return;
        }
     ...
     *
     * @param  \Hybrid\Contracts\Arrayable<array-key, TValue>|iterable<array-key, TKey>|TKey $keys
     * @return $this
     */
     ...
     *
     * @param  \Hybrid\Tools\Enumerable<array-key, TKey>|array<array-key, TKey>|string|null $keys
     * @return static
     */
    ...
    public function sole($key = null, $operator = null, $value = null)
    {
        ...
        $count = $items->count();
        if ($count === 0) {
            throw new \HBP_Disabler_Vendor\Hybrid\Tools\ItemNotFoundException();
        }
        if ($count > 1) {
            throw new \HBP_Disabler_Vendor\Hybrid\Tools\MultipleItemsFoundException($count);
        }
        return $items->first();
    }
    /**
