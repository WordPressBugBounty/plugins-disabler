...
namespace HBP_Disabler_Vendor\Hybrid\Filesystem;

use Closure;
use HBP_Disabler_Vendor\Hybrid\Contracts\Filesystem\Cloud as CloudFilesystemContract;
use HBP_Disabler_Vendor\Hybrid\Contracts\Filesystem\Filesystem as FilesystemContract;
use HBP_Disabler_Vendor\Hybrid\Http\File;
use HBP_Disabler_Vendor\Hybrid\Http\UploadedFile;
use HBP_Disabler_Vendor\Hybrid\Tools\Arr;
use HBP_Disabler_Vendor\Hybrid\Tools\Str;
use HBP_Disabler_Vendor\Hybrid\Tools\Traits\Conditionable;
use HBP_Disabler_Vendor\Hybrid\Tools\Traits\Macroable;
use HBP_Disabler_Vendor\League\Flysystem\FilesystemAdapter as FlysystemAdapter;
use HBP_Disabler_Vendor\League\Flysystem\FilesystemOperator;
use HBP_Disabler_Vendor\League\Flysystem\Ftp\FtpAdapter;
use HBP_Disabler_Vendor\League\Flysystem\Local\LocalFilesystemAdapter as LocalAdapter;
use HBP_Disabler_Vendor\League\Flysystem\PathPrefixer;
use HBP_Disabler_Vendor\League\Flysystem\PhpseclibV3\SftpAdapter;
use HBP_Disabler_Vendor\League\Flysystem\StorageAttributes;
use HBP_Disabler_Vendor\League\Flysystem\UnableToCopyFile;
use HBP_Disabler_Vendor\League\Flysystem\UnableToCreateDirectory;
use HBP_Disabler_Vendor\League\Flysystem\UnableToDeleteDirectory;
use HBP_Disabler_Vendor\League\Flysystem\UnableToDeleteFile;
use HBP_Disabler_Vendor\League\Flysystem\UnableToMoveFile;
use HBP_Disabler_Vendor\League\Flysystem\UnableToProvideChecksum;
use HBP_Disabler_Vendor\League\Flysystem\UnableToReadFile;
use HBP_Disabler_Vendor\League\Flysystem\UnableToRetrieveMetadata;
use HBP_Disabler_Vendor\League\Flysystem\UnableToSetVisibility;
use HBP_Disabler_Vendor\League\Flysystem\UnableToWriteFile;
use HBP_Disabler_Vendor\League\Flysystem\Visibility;
use HBP_Disabler_Vendor\PHPUnit\Framework\Assert as PHPUnit;
use HBP_Disabler_Vendor\Psr\Http\Message\StreamInterface;
use HBP_Disabler_Vendor\Symfony\Component\HttpFoundation\StreamedResponse;
use function HBP_Disabler_Vendor\Hybrid\Tools\throw_if;
/**
...
class FilesystemAdapter implements CloudFilesystemContract
{
    ...
    {
        return $this->driver->listContents($directory ?? '', $recursive)->filter(static fn(StorageAttributes $attributes) => $attributes->isFile())->sortByPath()->map(static fn(StorageAttributes $attributes) => $attributes->path())->toArray();
    }
    /**
    ...
    {
        return $this->driver->listContents($directory ?? '', $recursive)->filter(static fn(StorageAttributes $attributes) => $attributes->isDir())->map(static fn(StorageAttributes $attributes) => $attributes->path())->toArray();
    }
    /**
