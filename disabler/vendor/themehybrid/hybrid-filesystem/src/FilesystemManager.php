...
use HBP_Disabler_Vendor\Aws\S3\S3Client;
use Closure;
use HBP_Disabler_Vendor\Hybrid\Contracts\Filesystem\Factory as FactoryContract;
use HBP_Disabler_Vendor\Hybrid\Tools\Arr;
use HBP_Disabler_Vendor\League\Flysystem\AwsS3V3\AwsS3V3Adapter as S3Adapter;
use HBP_Disabler_Vendor\League\Flysystem\AwsS3V3\PortableVisibilityConverter as AwsS3PortableVisibilityConverter;
use HBP_Disabler_Vendor\League\Flysystem\Filesystem as Flysystem;
...
use HBP_Disabler_Vendor\League\Flysystem\Ftp\FtpConnectionOptions;
use HBP_Disabler_Vendor\League\Flysystem\Local\LocalFilesystemAdapter as LocalAdapter;
use HBP_Disabler_Vendor\League\Flysystem\PathPrefixing\PathPrefixedAdapter;
use HBP_Disabler_Vendor\League\Flysystem\PhpseclibV3\SftpAdapter;
use HBP_Disabler_Vendor\League\Flysystem\PhpseclibV3\SftpConnectionProvider;
use HBP_Disabler_Vendor\League\Flysystem\ReadOnly\ReadOnlyFilesystemAdapter;
use HBP_Disabler_Vendor\League\Flysystem\UnixVisibility\PortableVisibilityConverter;
...
class FilesystemManager implements FactoryContract
{
    ...
    public function createScopedDriver(array $config)
    {
        ...
        return $this->build(tap(is_string($config['disk']) ? $this->getConfig($config['disk']) : $config['disk'], static function (&$parent) use ($config) {
    ...
    protected function createFlysystem(FlysystemAdapter $adapter, array $config)
    {
        ...
        return new Flysystem($adapter, Arr::only($config, ['directory_visibility', 'disable_asserts', 'temporary_url', 'url', 'visibility']));
    }
    /**
